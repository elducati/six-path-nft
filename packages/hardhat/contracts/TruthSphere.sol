...
contract TruthSphere is ERC721Enumerable {
  ...
  function tokenURI(uint256 id) public view override returns (string memory) {
      ...
      string memory name = string(abi.encodePacked('TruthSphere #',id.toString()));
      ...
      string memory description = string(abi.encodePacked('This TruthSphere is color #',color[id].toColor(),crazyText,'!!!'));
  ...
  function generateSVGofTokenById(uint256 id) internal view returns (string memory) {
    ...
    string memory svg = string(abi.encodePacked(
      '<svg width="400" height="400" fill="#',color[id].toColor(),'" xmlns="http://www.w3.org/2000/svg">',
        renderTokenById(id),
      '</svg>'
    ));
  ...
  function renderTokenById(uint256 id) public view returns (string memory) {
    ...
    render = string(abi.encodePacked(render, TruthSeekerSVG.truthSphere(truthSeekerById[id])));
    return render;
  }
}