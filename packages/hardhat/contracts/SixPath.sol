...
contract SixPath is ERC721Enumerable {
  ...
  function tokenURI(uint256 id) public view override returns (string memory) {
      ...
      string memory name = string(abi.encodePacked('SixPath #',id.toString()));
      ...
      string memory description = string(abi.encodePacked('This SixPath is color #',color[id].toColor(),crazyText,'!!!'));
 ...
 // Visibility is `public` to enable it being called by other contracts for composition.
  function renderTokenById(uint256 id) public view returns (string memory) {
    ...
    render = string(abi.encodePacked(render, TruthSeekerSVG.sixPath(truthSeekerById[id])));
    return render;
  }
}